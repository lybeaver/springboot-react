...
export function showUserInfo (state={}, action) {
    switch(action.type){
        ...
        case SEARCH_DATA:
            ...
            return {...state, dataList: action.dataList, currentPage: action.currentPage, pageSize:action.pageSize, total:action.total}
        default:
            return state;
    }
}
...
export function searchData(dispatch, data) {
    ...
    axios.post("http://localhost:8080/public/searchUsers",data)
        .then(res=>{
            console.log("usersactionreducer",data)
...
export function doUserItems(items) {
    let result = [];
